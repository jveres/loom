...
import { attrRead, classRead, styleRead } from "loom/browser";
import { bindClass, bindStyle, h, remove } from "loom/dom";
// @vitest-environment happy-dom
...
describe("attr(el, name) — reactive reads", () => {
  it("tracks attribute set, change, and removal", async () => {
    ...
    const stop = effect(() => {
      seen.push(attrRead(el, "data-mode")());
    });
  ...
  it("pools per (element, attribute) and filters to subscribed names", async () => {
    const el = document.createElement("div");
    expect(attrRead(el, "hidden")).toBe(attrRead(el, "hidden"));
    expect(attrRead(el, "hidden")).not.toBe(attrRead(el, "title"));
    const hiddenSeen: Array<string | null> = [];
    const titleSeen: Array<string | null> = [];
    const stopHidden = effect(() => {
      hiddenSeen.push(attrRead(el, "hidden")());
    });
    const stopTitle = effect(() => {
      titleSeen.push(attrRead(el, "title")());
    });
  ...
  it("many elements share one observer with per-target filters", async () => {
    ...
    const stopA = effect(() => {
      aSeen.push(attrRead(a, "data-a")());
    });
    const stopB = effect(() => {
      bSeen.push(attrRead(b, "data-b")());
    });
  ...
  it("resyncs on re-subscribe after changing while unobserved", () => {
    const el = document.createElement("div");
    const stop1 = effect(() => attrRead(el, "data-x")());
    stop1(); // observer disconnected
    el.setAttribute("data-x", "late");
    let now: string | null = null;
    const stop2 = effect(() => {
      now = attrRead(el, "data-x")();
    });
  ...
  it("adds targets incrementally and batches target removals", async () => {
    ...
    const stops = elements.map((el) => effect(() => attrRead(el, "data-x")()));
