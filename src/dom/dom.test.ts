...
describe("loom DOM list", () => {
  ...
  it("moves existing keys with moveBefore when the platform has it", () => {
    ...
    (
      root as unknown as {
        moveBefore: (n: Node, ref: Node | null) => void;
      }
    ).moveBefore = (node, ref) => {
...
describe("loom DOM props and bindings", () => {
  it("applies key, static, false/aria, event, and reactive props", () => {
    ...
    const el = h("button", {
      ...
      title: () => title(), // explicit attr binding
  ...
  it("fires an installed tap but not a drag, and resets on pointercancel", () => {
    let taps = 0;
    const el = h("button");
    onTap(el, () => {
      taps++;
    ...
    send("pointerdown", 10, 10);
    el.dispatchEvent(
      new PointerEvent("pointercancel", { isPrimary: true, pointerId: 1 }),
    );
  ...
  it("binds tap imperatively too", () => {
    ...
    el.dispatchEvent(
      new PointerEvent("pointerdown", { isPrimary: true, pointerId: 1 }),
    );
    el.dispatchEvent(
...
describe("loom DOM onunmount", () => {
  ...
  it("combines a lifecycle prop with explicit tap installation", () => {
    const cleanup = vi.fn();
    const tapped = vi.fn();
