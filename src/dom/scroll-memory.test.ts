...
import { remove, scrollMemory } from "./index.js";
...
describe("scrollMemory", () => {
  it("persists per key from the host's scroll events and restores after the swap", async () => {
    ...
    const cells = keyedStates();
    const memory = scrollMemory(host, (key) => cells.cell(`scroll:${key}`, 0));
    ...
    expect(cells.cell("scroll:a", 0)()).toBe(120);

    memory.restore("b"); // the swap clamps to 0 and fires BEFORE the restore microtask
    host.scrollTop = 0;
    host.dispatchEvent(new Event("scroll"));
    expect(cells.cell("scroll:a", 0)()).toBe(120); // the clamp never persisted
    await microtask();
  ...
  it("dies with the host: no persistence, no pending restore", async () => {
    ...
    document.body.append(host);
    const cells = keyedStates();
    const memory = scrollMemory(host, (key) => cells.cell(key, 0));
    cells.cell("a", 0)(77);
    memory.restore("a");
    remove(host);
    await microtask();
