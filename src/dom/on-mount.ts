...
// this module into every h() bundle, accepted deliberately: the lifecycle pair belongs in the
// baseline prop vocabulary.)
//
...
function enqueue(
  ...
  fns.add(fn);
  const owner = pool;
  pool.observer ??= observeDocumentTree(document, () => sweep(owner));
  return pool;
}
