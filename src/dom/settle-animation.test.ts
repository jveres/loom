...
import { afterAnimation } from "loom/motion";
// @vitest-environment happy-dom
...
describe("settleAnimation", () => {
  it("settles on a MICROTASK when nothing will animate (animation: none)", async () => {
    ...
    const settled = vi.fn();
    afterAnimation(el, settled);
    expect(settled).not.toHaveBeenCalled(); // never synchronous
    await microtask();
    expect(settled).toHaveBeenCalledOnce();
    // An unrelated event cannot complete a wait with no selected animation.
    const early = vi.fn();
    afterAnimation(el, early);
  ...
  it("the Stop abandons the wait, and a dead node never settles", () => {
    ...
    document.body.append(el);
    const settled = vi.fn();
    const stop = afterAnimation(el, settled);
    stop();
    el.dispatchEvent(animationEvent("animationend", "enter"));
