...
import { _ as d, g as te, h as ne, m as re, p as f, r as p } from "./dom-CUhC3JWe.js";
...
var D = null, _t = null, vt = null, O = "all", yt = null, k = null, bt = null, A = null, xt = null, j = [], M = [], N = !1, P = !1, F = "", St = 0, Ct = -1, I = -1, wt = null;
function Tt(e) {
	wt = e;
}
function Et(e) {
...
function Ot() {
	xt && (xt.classList.toggle("inactive", !P), xt.classList.toggle("off", N), xt.title = P ? N ? "Paused" : "Live — capturing" : "Trace");
}
function kt() {
	Mt(), D = m({
		rowHeight: ft,
	...
	}), A.append(y(Ce, 12)), f(A, () => Rt(!N));
	let e = /* @__PURE__ */ h("button", {
		type: "button",
		class: "li-tr-btn",
		title: "Clear the trace"
	});
	e.append(y(Se, 12)), f(e, () => Nt());
	...
	}), k = /* @__PURE__ */ h("div", { class: "li-tr-scroll" }), k.append(D.el), bt = te(k), k.addEventListener("pointerover", (e) => {
...
function Ft() {
	...
	O === "all" && e.sort((e, t) => s(e.s).t - s(t.s).t), Vt = !1;
	...
	j.length > mt && (j.length = mt), M.length > mt && (M.length = mt), Ct = ((F ? M : j)[0]?.seq ?? -1) === r ? -1 : r, L();
}
function It() {
	Ft(), L(), requestAnimationFrame(() => D?.refresh());
}
function Lt() {
	jt(), D = null, yt = null, k = null, bt?.(), bt = null, A = null, xt = null, j = [], M = [], Bt.clear(), Vt = !1, Ct = -1, N = !1, P = !1, F = "", O = "all", I = -1, wt = null;
}
function Rt(e) {
	N = e, A?.replaceChildren(y(e ? we : Ce, 12)), Ot(), yt?.classList.toggle("li-tr-paused", e), e || Ft();
}
function L() {
	let e = F ? M : j;
...
function $n() {
	...
	H(e, "stroke-dasharray", W(() => `${Yt * An / 100} ${Jt}`)), H(e, "class", W(() => `li-garc h-${jn}`));
	...
	return t.append(re(W(() => String(An)), _)), H(t, "class", W(() => `li-gnum h-${jn}`)), [e, t];
}
function er() {
...
function q(e, t, n = "", r = "") {
	...
	return i.append(re(W(t), _)), /* @__PURE__ */ g("div", {
		class: "li-stat",
...
function nr() {
	...
	e.append(re(W(() => B ? `${Math.round(z)} fps` : "— fps"), _)), H(e, "class", W(() => `li-perfh-fps ${Pn}`));
	let t = /* @__PURE__ */ h("div", {
		class: "li-hlabel",
		title: J.health
	});
	t.append(re(W(() => B ? Mn.toUpperCase() : "LOADING"), _)), H(t, "class", W(() => Nn ? `li-hlabel h-${jn}` : "li-hlabel"));
	let n = /* @__PURE__ */ g("div", {
		class: "li-hstats",
		children: [t, q("lag", () => `${xn.toFixed(0)} · pk ${Sn.toFixed(0)} ms`, "lo", J.lag)]
	});
	...
	}, J.blocked)), n.append(rr("CLS", () => U(Tn).toFixed(2), () => {
		let e = U(Tn);
		return e < .1 ? "h-ok" : e < .25 ? "h-warn" : "h-bad";
	}, J.cls)), n.append(rr("LCP", () => {
		let e = U(En);
		return e ? `${(e / 1e3).toFixed(2)} s` : "—";
	}, () => Gn(U(En), 2500, 4e3), J.lcp)), n.append(rr("INP", () => {
		let e = U(Dn);
		return e ? `${e.toFixed(0)} ms` : "—";
	}, () => Gn(U(Dn), 200, 500), J.inp)), /* @__PURE__ */ g("div", {
		class: "li-pane",
		children: [
			/* @__PURE__ */ g("div", {
				class: "li-perfh",
			...
			q("frame time", () => `${bn.toFixed(1)} ms`, "", J.frameTime),
			ir() ? ar() : null,
			q("writes / s", () => K(ln), "hi", J.writes),
			q("reads / s", () => K(cn), "hi", J.reads),
			q("computeds / s", () => K(un), "", J.computedsRate),
			q("effect runs / s", () => K(dn), "lo", J.effectRuns),
			q("flushes / s", () => K(fn), "lo", J.flushes),
			q("effects / flush", () => String(hn), "", J.effectsPerFlush),
			q("flush time", () => `${gn.toFixed(1)} ms`, "", J.flushTime),
			q("creates / s", () => K(pn), "lo", J.creates),
			q("disposes / s", () => K(mn), "lo", J.disposes),
			q("states", () => String(Fn), "", J.states),
			q("computeds", () => String(In), "", J.computeds),
			rr("unread", () => String(Hn), () => Hn > 0 ? "h-warn" : "", J.unread),
			q("effects", () => String(Ln), "", J.effects),
			q("views", () => String(Rn), "", J.views),
			q("sources", () => String(zn), "", J.sources),
			q("scopes", () => String(Bn), "", J.scopes),
			q("channels", () => String(Vn), "", J.channels)
		]
	});
}
function rr(e, t, n, r = "") {
...
function or() {
	...
	cn = G(cn, t), ln = G(ln, n), dn = G(dn, r), un = G(un, i), pn = G(pn, a), mn = G(mn, o), fn = G(fn, c?.count ?? 0);
...
function ur(t) {
	$t = t.activeTab, en = t.isMinimized, on = l([
		...
		c.create,
		c.dispose
	]), sn = l([c.flush], "samples"), R = n(or, Xt, _), ae(() => {
		R?.(), e(sr);
	}, {
		defer: !0,
		maxStale: Xt
	...
	let r;
	return rn = o(() => {
		Tn = a(qn, 0, _), En = a(Jn, 0, _), Dn = a(Yn, 0, _), On = a(Zn, 0, _), ir() && (kn = n(() => ir()?.usedJSHeapSize ?? 0, 5e3, _)), r = nr();
	}, _), lr(), r;
}
function dr() {
	rn?.pause();
...
function pr() {
	on?.stop(), on = null, sn?.stop(), sn = null, R?.stop(), R = null, tn != null && clearInterval(tn), tn = null, document.removeEventListener("visibilitychange", cr), nn != null && cancelAnimationFrame(nn), nn = null, rn?.stop(), rn = null, kn = Tn = En = Dn = On = null, an = 0, cn = ln = un = dn = fn = 0, pn = mn = 0, hn = gn = 0, z = 0, B = !1, _n = vn = yn = bn = 0, V.length = 0, xn = Sn = 0, wn = !1, Nn = !1, An = 100, jn = Mn = Pn = "", Fn = In = Ln = Rn = 0, zn = Bn = Vn = Hn = 0;
}
//#endregion
...
], Y = null, gr = null, X = null, _r = null, vr = [], yr = null, Z = null, br = /* @__PURE__ */ new Map(), xr = null, Sr = [
	1e3,
	5e3,
	25e3
], Cr = null;
...
function Or(e, t) {
	e.addEventListener("pointerdown", (n) => {
		...
		e.style.cursor = "grabbing", Dr(e, t, n, (n, o) => {
			let { left: s, top: c } = wr(t, e.offsetHeight || 40, o.left + n.clientX - r, o.top + n.clientY - i);
...
function jr(e = document.body) {
	...
	Z = r("stats", _);
	let t = Q().theme(), n = /* @__PURE__ */ h("span", { class: "li-menu-val" }), a = () => {
		Y?.setAttribute("data-theme", t), gr?.setAttribute("data-theme", t), n.innerHTML = ue(mr[t], 13), s.title = `Theme: ${t} (click to cycle)`;
	}, s = /* @__PURE__ */ g("button", {
		type: "button",
		class: "li-menu-item",
		title: "Click to change theme",
		children: [/* @__PURE__ */ h("span", { children: "Theme" }), n]
	});
	f(s, () => {
		let e = [
			"system",
	...
	f(d, () => {
		l = Sr[(Sr.indexOf(l) + 1) % Sr.length] ?? 1e3, Q().logSize(l), u();
	}), c.append(d), u();
	let ne = () => {
		c.hidden = !0;
	...
	let m = /* @__PURE__ */ h("button", { type: "button" }), ie = (e) => {
		m.title = e ? "Expand" : "Collapse", m.replaceChildren(Ar(e ? fe : de));
	}, oe = Q().min();
	ie(oe), f(m, () => {
		let e = !!Y?.classList.toggle("li-min");
		ie(e), Q().min(e), e ? yr?.pause() : yr?.resume(), Dt(!e && Z?.() === "trace");
	});
	...
	for (let e of hr) {
		let t = e.id === "stats" ? le : e.id === "graph" ? Re() : kt();
	...
	Y = /* @__PURE__ */ g("div", { children: [
		...
		X,
		ve
	] }), Y.id = v, oe && Y.classList.add("li-min"), a(), Or(ce, Y), kr(ve, Y), _r = (e) => {
		let t = e.target instanceof Node ? e.target : null;
		!c.hidden && (t === null || !c.contains(t)) && e.target !== p && ne();
	}, document.addEventListener("pointerdown", _r), e.append(Y), document.body.append(c);
	...
	if (ye && (Y.style.width = `${Math.max(240, Math.min(ye.width, window.innerWidth - 16))}px`, Y.style.height = `${Math.max(160, Math.min(ye.height, window.innerHeight - 16))}px`), be) {
		let { left: e, top: t } = Tr(Y, be.left, be.top);
		Y.style.left = `${e}px`, Y.style.top = `${t}px`, Y.style.right = "auto", Y.style.bottom = "auto";
	}
	ae(() => {
		let e = Z?.();
		xr && xr !== e && X && br.set(xr, X.scrollTop), e === "stats" ? fr() : dr(), e !== "graph" && ot();
		for (let t of hr) {
		...
		if (e && X) {
			let t = br.get(e) ?? 0, n = Math.max(0, X.scrollHeight - X.clientHeight);
			X.scrollTop = Math.min(t, n), e === "graph" ? ct() : e === "trace" && It();
		}
		Dt(e === "trace" && Y?.classList.contains("li-min") !== !0), xr = e ?? null;
	}), vr.push(te(X), te(he, { axis: "x" }));
}
function Mr() {
	pr();
	for (let e of vr) e();
	vr.length = 0, oe(), yr?.stop(), yr = null, _r && document.removeEventListener("pointerdown", _r), _r = null, gr?.remove(), gr = null, Y?.remove(), Y = null, X = null, Z = null, br.clear(), xr = null, dt(), Lt();
}
function Nr() {
	return Y !== null;
}
...
//#endregion
export { Nr as inspectorMounted, jr as mountInspector, Pr as toggleInspector, Mr as unmountInspector };
